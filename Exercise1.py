...
#         json.dump(content,f)#function to ensure that we’re wcat riting out valid JSON
'''
...
# else:
# receipts = glob.glob('./new/receipt-[0-9-]*.json') # only print two digit number. us re module. 
# subtotal = 0.0
...
'''
One actual improvement that we can make to our process_receipts.py 
file is that we can use a single function call to go from our path variable to the destination that we want.
This section: 
'''
...
'''
If we have a project that relies on boto3, 
we probably want to keep track of that dependency somewhere, 
and pip can facilitate this through a “requirements file” traditionally called requirements.txt.
 If we’ve already installed everything manually,
  then we can dump the current dependency state using the freeze subcommand that pip provides.
'''

'''
We can only have one version of a package installed at a given time,
 and this can sometimes be a headache
if we have multiple projects that require different versions of the same dependency. 
This is where virtualenv comes into play and allows us to create sandboxed Python environments.

Virtualenv or Venv
Virtualenvs allow you to create sandboxed Python environments. 
In Python 2, you need to install the virtualenv package to do this, 
but with Python 3 it’s been worked in under the module name of venv.
...
* To use a virtualenv, we need to activate it. 
* We do this by sourcing an activate file in the virtualenv’s bin directory:

$ source venvs/experiment/bin/activate
(experiment) ~ $

*To remove the virtualenv’s contents from our $PATH,
* we will utilize the deactivate script that the virtualenv provided.
'''
